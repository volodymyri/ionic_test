Ext.define('criterion.view.recruiting.jobs.JobCandidateDetails', function() {
    ...
    return {
        ...
        alias : 'widget.criterion_recruiting_jobs_job_candidate_details',

        extend : 'criterion.ux.form.Panel',

        requires : [
            'criterion.controller.recruiting.jobs.JobCandidateDetails',
        ...
        controller : {
            type : 'criterion_recruiting_jobs_job_candidate_details'
        },
        ...
        viewModel : {
            ...
            formulas : {
                ...
                jobPostingStatus : data => data('_cjp.jobPosting.status'),
                jobPostingTitle : data => data('_cjp.jobPosting.title'),
                jobPostingRequisitionCode : data => data('_cjp.jobPosting.requisitionCode')
            }
        },
                ...
                items : [
                    ...
                    },
                    {
                        ...
                        items : [
                            {
                                xtype : 'component',
                                cls : 'job-posting-title',
                                bind : {
                                    html : '{jobPostingTitle}'
                                }
                            },
                            {
                                xtype : 'container',
                                ...
                                items : [
                                    {
                                        xtype : 'component',
                                        bind : {
                                            html : '<span class="criterion-text-gray">#{jobPostingRequisitionCode}</span>'
                                        }
                                    },
                                    {
                                        xtype : 'container',
                                        ...
                                        items : [
                                            {
                                                xtype : 'component',
                                                bind : {
                                                    html : '<div style="text-align:right;">{jobPostingStatus}</div>'
                                                }
                                            }
                                        ]
                                    }
                        ...
                        columns : [
                            {
                                ...
                                tpl : Ext.create('Ext.XTemplate', '{[this.getInfo(values)]}'
                                    + '<div>{appliedDate:date("m/d/Y")} - {[this.getStatusCode(values.candidateStatusCd)]}</div>'
                                    + '<fieldset class="rating">'
                                    + '{[this.generateRating(5, values.rating)]}'
                                    + '</fieldset>', {
                                    getInfo : function(val) {
                                        return Ext.util.Format.format(
                                            ...
                                            Ext.String.htmlEncode(val.candidate.location)
                                        );
                                    },
                                    generateRating : (count, rating) => {
                                        let html = '';

                                        for (let i = 0; i < count; i++) {
                                            html += ''
                                                + Ext.util.Format.format('<label class="{0}"></label>', i >= count - rating ? 'selected' : '')
                                                + Ext.util.Format.format('<label class="half {0}"></label>', count - rating > i && count - rating < i + 1 ? 'selected' : '');
                                        }

                                        return html;
                                    },
                                    getStatusCode : statusCd => {
                                        let codeRecord = criterion.CodeDataManager.getCodeDetailRecord('id', statusCd, DICT.CANDIDATE_STATUS);

                                        return (codeRecord && codeRecord.get('description')) || '';
                                    }
                                }),
                                flex : 1
                            }
