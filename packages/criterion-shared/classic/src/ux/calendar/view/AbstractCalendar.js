/**
 * @class criterion.ux.calendar.view.AbstractCalendar
 ...
 */
Ext.define('criterion.ux.calendar.view.AbstractCalendar', {
     ...
     * {@link criterion.ux.calendar.EventRecord}) to populate the calendar views with events. Internally this method
     ...
     * @param {criterion.ux.calendar.view.AbstractCalendar} this
     */
     ...
     * @param {criterion.ux.calendar.view.AbstractCalendar} this
     * @param {criterion.ux.calendar.EventRecord} rec The {@link criterion.ux.calendar.EventRecord record} for the event that was clicked on
     ...
     * @param {criterion.ux.calendar.view.AbstractCalendar} this
     * @param {criterion.ux.calendar.EventRecord} rec The {@link criterion.ux.calendar.EventRecord record} for the event that the cursor is over
     ...
     * @param {criterion.ux.calendar.view.AbstractCalendar} this
     * @param {criterion.ux.calendar.EventRecord} rec The {@link criterion.ux.calendar.EventRecord record} for the event that the cursor exited
     ...
     * @param {criterion.ux.calendar.view.AbstractCalendar} this
     * @param {criterion.ux.calendar.EventRecord} rec The {@link criterion.ux.calendar.EventRecord record} for the event that was moved with
     ...
     * @param {criterion.ux.calendar.view.AbstractCalendar} this
     * @param {criterion.ux.calendar.EventRecord} rec The {@link criterion.ux.calendar.EventRecord record} for the event that was deleted
     */
    ...
    getWeekCount: function() {
        var days = criterion.ux.calendar.util.Date.diffDays(this.viewStart, this.viewEnd);
            ...
            for (d = 0; d < this.dayCount; d++) {
                if (evtsInView.getCount() > 0) {
                    var evts = evtsInView.filterBy(function(rec) {
                        var startDt = Ext.Date.clearTime(rec.data[criterion.ux.calendar.data.EventMappings.StartDate.name], true),
                            startsOnDate = dt.getTime() == startDt.getTime(),
                            spansFromPrevView = (w == 0 && d == 0 && (dt > rec.data[criterion.ux.calendar.data.EventMappings.StartDate.name]));
                ...
                dt = criterion.ux.calendar.util.Date.add(dt, {days: 1});
            }
        }
    ...
    prepareEventGrid: function(evts, w, d) {
        ...
        evts.each(function(evt) {
            ...
            days = criterion.ux.calendar.util.Date.diffDays(
            criterion.ux.calendar.util.Date.max(me.viewStart, evt.data[M.StartDate.name]),
            criterion.ux.calendar.util.Date.min(me.viewEnd, evt.data[M.EndDate.name])) + 1;

            if (days > 1 || criterion.ux.calendar.util.Date.diffDays(evt.data[M.StartDate.name], evt.data[M.EndDate.name]) > 1) {
    ...
    onEventDrop: function(rec, dt) {
        if (criterion.ux.calendar.util.Date.compare(rec.data[criterion.ux.calendar.data.EventMappings.StartDate.name], dt) === 0) {
            // no changes
            return;
        }
        var diff = dt.getTime() - rec.data[criterion.ux.calendar.data.EventMappings.StartDate.name].getTime();
        rec.set(criterion.ux.calendar.data.EventMappings.StartDate.name, dt);
        rec.set(criterion.ux.calendar.data.EventMappings.EndDate.name, criterion.ux.calendar.util.Date.add(rec.data[criterion.ux.calendar.data.EventMappings.EndDate.name], {millis: diff}));
    ...
    onCalendarEndDrag: function(start, end, onComplete) {
        if (start && end) {
            ...
            var o = {};
            o[criterion.ux.calendar.data.EventMappings.StartDate.name] = start;
            o[criterion.ux.calendar.data.EventMappings.EndDate.name] = end;
    ...
    onUpdate: function(ds, rec, operation) {
        ...
        if (operation == Ext.data.Record.COMMIT) {
            ...
            if (this.enableFx && this.enableUpdateFx) {
                this.doUpdateFx(this.getEventEls(rec.data[criterion.ux.calendar.data.EventMappings.EventId.name]), {
                    scope: this
                });
            }
        }
    ...
    onAdd: function(ds, records, index) {
        ...
        if (this.enableFx && this.enableAddFx) {
            this.doAddFx(this.getEventEls(rec.data[criterion.ux.calendar.data.EventMappings.EventId.name]), {
                scope: this
            });
        }
    },
    ...
    onRemove: function(ds, recs) {
        var name = criterion.ux.calendar.data.EventMappings.EventId.name,
            i, len, rec, els;
    ...
    isEventVisible: function(evt) {
        var M = criterion.ux.calendar.data.EventMappings,
            ...
            evEnd = criterion.ux.calendar.util.Date.add(data[M.EndDate.name], {seconds: -1}).getTime();
    ...
    isOverlapping: function(evt1, evt2) {
        ...
        M = criterion.ux.calendar.data.EventMappings,
        start1 = ev1[M.StartDate.name].getTime(),
        end1 = criterion.ux.calendar.util.Date.add(ev1[M.EndDate.name], {seconds: -1}).getTime(),
        start2 = ev2[M.StartDate.name].getTime(),
        end2 = criterion.ux.calendar.util.Date.add(ev2[M.EndDate.name], {seconds: -1}).getTime();
    ...
    setViewBounds: function(startDate) {
        var start = startDate || this.startDate,
            ...
            Dt = criterion.ux.calendar.util.Date;
     ...
     * of {@link #criterion.ux.calendar.EventRecord EventRecord} objects
     */
    sortEventRecordsForDay: function(evts) {
        ...
        evts.sortBy(Ext.bind(function(evtA, evtB) {
            ...
            if (this.spansHavePriority) {
                ...
                var diff = criterion.ux.calendar.util.Date.diffDays;
    ...
    moveNext: function(noRefresh) {
        return this.moveTo(criterion.ux.calendar.util.Date.add(this.viewEnd, {days: 1}));
    },
    ...
    movePrev: function(noRefresh) {
        var days = criterion.ux.calendar.util.Date.diffDays(this.viewStart, this.viewEnd) + 1;
    ...
    moveMonths: function(value, noRefresh) {
        return this.moveTo(criterion.ux.calendar.util.Date.add(this.startDate, {months: value}), noRefresh);
    },
    ...
    moveWeeks: function(value, noRefresh) {
        return this.moveTo(criterion.ux.calendar.util.Date.add(this.startDate, {days: value * 7}), noRefresh);
    },
    ...
    moveDays: function(value, noRefresh) {
        return this.moveTo(criterion.ux.calendar.util.Date.add(this.startDate, {days: value}), noRefresh);
    },
    ...
    /**
     * Sets the event store used by the calendar to display {@link criterion.ux.calendar.EventRecord events}.
    ...
    getEventRecord: function(id) {
        var idx = this.store.find(criterion.ux.calendar.data.EventMappings.EventId.name, id);
