 ...
 */
Ext.define('criterion.ux.calendar.view.MonthDayDetail', {
    ...
    requires: [
        ...
        'criterion.ux.calendar.view.AbstractCalendar'
    ],
    ...
    refresh: function() {
        ...
        evts = this.store.queryBy(function(rec) {
            var thisDt = Ext.Date.clearTime(this.date, true).getTime(),
                recStart = Ext.Date.clearTime(rec.data[criterion.ux.calendar.data.EventMappings.StartDate.name], true).getTime(),
                startsOnDate = (thisDt == recStart),
                spansDate = false;

            if (!startsOnDate) {
                var recEnd = Ext.Date.clearTime(rec.data[criterion.ux.calendar.data.EventMappings.EndDate.name], true).getTime();
        ...
        evts.each(function(evt) {
            ...
            item._renderAsAllDay = item[M.IsAllDay.name] || criterion.ux.calendar.util.Date.diffDays(item[M.StartDate.name], item[M.EndDate.name]) > 0;
            item.spanLeft = criterion.ux.calendar.util.Date.diffDays(item[M.StartDate.name], this.date) > 0;
            item.spanRight = criterion.ux.calendar.util.Date.diffDays(this.date, item[M.EndDate.name]) > 0;
