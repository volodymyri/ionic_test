/**
 * @class criterion.ux.calendar.view.Month
 * @extends criterion.ux.calendar.CalendarView
 * <p>Displays a calendar view by month. This class does not usually need ot be used directly as you can
 * use a {@link criterion.ux.calendar.CalendarPanel CalendarPanel} to manage multiple calendar views at once including
 * the month view.</p>
 * @constructor
 * @param {Object} config The config object
 */
Ext.define('criterion.ux.calendar.view.Month', {
    extend: 'criterion.ux.calendar.view.AbstractCalendar',
    alias: 'widget.monthview',
    
    requires: [
        'Ext.XTemplate',
        'criterion.ux.calendar.template.Month',
        'criterion.ux.calendar.util.WeekEventRenderer',
        'criterion.ux.calendar.view.MonthDayDetail'
    ],
     ...
     * True to display an extra column before the first day in the calendar that links to the {@link criterion.ux.calendar.WeekView view}
     ...
     * @param {criterion.ux.calendar.view.Month} this
    ...
    initDD: function() {
        ...
        this.dragZone = new criterion.ux.calendar.dd.DragZone(this.el, cfg);
        this.dropZone = new criterion.ux.calendar.dd.DropZone(this.el, cfg);
    },
    ...
    afterRender: function() {
        if (!this.tpl) {
            this.tpl = new criterion.ux.calendar.template.Month({
                id: this.id,
    ...
    getEventTemplate: function() {
        if (!this.eventTpl) {
            ...
            new Ext.XTemplate(
                '<div id="{_elId}" class="{_selectorCls} {_colorCls} {spanCls} ext-cal-evt ext-cal-evr {_fullDayCls}">',
                    '<tpl if="_hasAdditionalMark">',
                        '<div class="ext-cal-additional-mark" style="background-color: {_additionalMarkColor}">&nbsp;</div>',
                    '</tpl>',
                    body,
                    '<tpl if="_fullDay">',
                        '<div class="ext-cal-full-day-mark">Full Day</div>',
                    '</tpl>',
                    '<tpl if="_modifiedFullDay && !_fullDay">',
                        '<div class="ext-cal-normal-day-mark">{_startTime:date("ga")} -> {_endTime:date("ga")}</div>',
                    '</tpl>',
                    '<tpl if="_additionalText">',
                        '<div class="ext-cal-additional-text">{_additionalText}</div>',
                    '</tpl>',
                '</div>'
            )
            : new Ext.XTemplate(
    ...
    getTemplateEventData: function(evt) {
        ...
        return Ext.applyIf({
            ...
            _hasAdditionalMark : /^#/.test(additionalMarkColor),
            _additionalMarkColor : additionalMarkColor,
            _modifiedFullDay : evt['-isModifiedFullDay'],
            _fullDay : evt['-isFullDay'],
            _fullDayCls : evt['-isFullDay'] ? 'ext-cal-full-day' : '',
            _startTime : evt['-startTime'],
            _endTime : evt['-endTime'],
            _additionalText : evt['-additionalText']
        },
        evt);
    },

    // private
    renderEventCallback: function(evt) {
        var selector = Ext.util.Format.format('#{0}-day-{1}{2}', this.getId(), Ext.Date.format(evt.StartDate, 'Ymd'), this.daySelector);

        if (evt.highlightDayCls) {
            Ext.select(selector).addCls(evt.highlightDayCls);
        }
    },
    ...
    renderItems: function() {
        criterion.ux.calendar.util.WeekEventRenderer.render({
            ...
            renderCallback: this.highlightEventDay ? Ext.bind(this.renderEventCallback, this) : null
        });
    ...
    getEventHeight : function() {
        if (!this.eventHeight) {
            ...
            if(evt){
                ...
                return 28; // no events rendered, so try setting this.eventHeight again later
            }
        }
    ...
    getDayAt: function(x, y) {
        var box = this.el.getBox(),
            ...
            days = (dayT * 7) + dayL,
            dt = criterion.ux.calendar.util.Date.add(this.viewStart, {days: days});
        return {
            date: dt,
            el: this.getDayEl(dt)
        };
    ...
    onClick: function(e, t) {
        ...
        if (criterion.ux.calendar.view.Month.superclass.onClick.apply(this, arguments)) {
