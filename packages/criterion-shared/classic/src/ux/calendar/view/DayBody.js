/**S
 * @class criterion.ux.calendar.view.DayBody
 * @extends criterion.ux.calendar.view.AbstractCalendar
 * <p>This is the scrolling container within the day and week views where non-all-day events are displayed.
 * Normally you should not need to use this class directly -- instead you should use {@link criterion.ux.calendar.DayView DayView}
 * which aggregates this class and the {@link criterion.ux.calendar.DayHeaderView DayHeaderView} into the single unified view
 * presented by {@link criterion.ux.calendar.CalendarPanel CalendarPanel}.</p>
 * @constructor
 * @param {Object} config The config object
 */
Ext.define('criterion.ux.calendar.view.DayBody', {
    extend: 'criterion.ux.calendar.view.AbstractCalendar',
    alias: 'widget.daybodyview',

    requires: [
        'Ext.XTemplate',
        'criterion.ux.calendar.template.DayBody',
        'criterion.ux.calendar.data.EventMappings',
        'criterion.ux.calendar.dd.DayDragZone',
        'criterion.ux.calendar.dd.DayDropZone'
    ],
     ...
     * @param {criterion.ux.calendar.view.DayBody} this
     * @param {criterion.ux.calendar.EventRecord} rec The {@link criterion.ux.calendar.EventRecord record} for the event that was resized
    ...
    initDD: function() {
        ...
        this.dragZone = new criterion.ux.calendar.dd.DayDragZone(this.el, Ext.apply({
            view: this,
            containerScroll: true
        },
        cfg));

        this.dropZone = new criterion.ux.calendar.dd.DayDropZone(this.el, Ext.apply({
            view: this
        },
        cfg));
    },
    ...
    afterRender: function() {
        if (!this.tpl) {
            this.tpl = new criterion.ux.calendar.template.DayBody({
                id: this.id,
    ...
    onEventResize: function(rec, data) {
        var D = criterion.ux.calendar.util.Date,
        start = criterion.ux.calendar.data.EventMappings.StartDate.name,
        end = criterion.ux.calendar.data.EventMappings.EndDate.name;
    ...
    getEventBodyMarkup: function() {
        if (!this.eventBodyMarkup) {
            this.eventBodyMarkup = [
                ...
                '<tpl if="_timeLine">',
                '<div class="time_line">{timeLine}</div>',
                '</tpl>'
            ].join('');
        }
    ...
    getEventTemplate: function() {
        if (!this.eventTpl) {
            ...
            new Ext.XTemplate(
                '<div id="{_elId}" class="{_selectorCls} {_colorCls} {_allDayCls} ext-cal-evt ext-cal-evr" style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;">',
                '<div class="ext-evt-bd">', this.getEventBodyMarkup(), '</div>',
                '<div class="ext-evt-rsz"><div class="ext-evt-rsz-h">&#160;</div></div>',
                '</div>'
            )
            : new Ext.XTemplate(
                '<div id="{_elId}" class="ext-cal-evt {_selectorCls} {_allDayCls} {_colorCls}-x" style="left: {_left}%; width: {_width}%; top: {_top}px;">',
     ...
     * {@link criterion.ux.calendar.EventRecord}) to populate the calendar views with <strong>all-day</strong> events.
    ...
    getTemplateEventData : function(evt, date) {
        var selector = this.getEventSelectorCls(evt[criterion.ux.calendar.data.EventMappings.EventId.name]),
            data = {},
            M = criterion.ux.calendar.data.EventMappings;

        this.getTemplateEventBox(evt, date);

        data._selectorCls = selector;
        data._colorCls = 'ext-color-' + (evt[M.CalendarId.name] || '0') + (evt._renderAsAllDay ? '-ad': '');
        data._allDayCls = evt._renderAsAllDay ? 'allDay' : 'defBlock';
        data._elId = selector + (evt._weekIndex ? '-' + evt._weekIndex : '') + '-' + (date.getTime());
        ...
        data._timeLine = !evt._renderAsAllDay;
        data.timeLine = Ext.Date.format(evt[M.StartDate.name], 'g:ia ') + ' - ' + Ext.Date.format(evt[M.EndDate.name], 'g:ia ');
    ...
    // private
    getTemplateEventBox : function(evt, date) {
        var heightFactor = 0.7,
            M = criterion.ux.calendar.data.EventMappings,
            start = evt[M.StartDate.name],
            end = evt[M.EndDate.name],
            startMins,
            endMins,
            diffMins,
            diffStart = criterion.ux.calendar.util.Date.diffDays(start, date),
            diffEnd = criterion.ux.calendar.util.Date.diffDays(date, end);

        if (evt.IsAllDay) {
            start = Ext.Date.clearTime(start, true);
            evt[M.StartDate.name] = start;

            end.setHours(23);
            end.setMinutes(59);
            end.setSeconds(59);
            evt[M.EndDate.name] = end;
            evt._renderAsAllDay = true;
        } else {
            if (diffStart !== 0) {
                start = Ext.Date.clearTime(date, true);
                evt._renderAsAllDay = true;
            }
            if (diffEnd !== 0) {
                end = Ext.Date.clone(date);
                end.setHours(23);
                end.setMinutes(59);
                end.setSeconds(59);
                evt._renderAsAllDay = true;
        ...
        evt._height = Math.max((diffMins * heightFactor), 20) + 15;
    },
    ...
    renderItems: function() {
        ...
        for (; day < this.dayCount; day++) {
            date = criterion.ux.calendar.util.Date.add(this.viewStart, {days : day});
            ...
            for (; ev < ct; ev++) {
                ...
                // if (item._renderAsAllDay) { // blocked
                //     continue;
                // }
                Ext.apply(item, {
                    cls: 'ext-cal-ev',
                ...
                evts.push({
                    data : this.getTemplateEventData(item, date),
                    date : date
                });
            }
        }
        ...
        for (; day < this.dayCount; day++) {
            date = criterion.ux.calendar.util.Date.add(this.viewStart, {days : day});
            evtsd = Ext.Array.filter(evts, function(evvalue) {
                return date.getTime() === evvalue.date.getTime();
            });

            // clean
            Ext.get(me.getDayId(date)).select('*').destroy();
            ...
            for (i = 0; i < l; i++) {
                ...
                if (evt._overlap !== undefined) {
                    ...
                    evt._width = colWidth - 0.5;
                    evt._left = colWidth * evt._overcol;
                }
                markup = this.getEventTemplate().apply(evt);
                target = this.id + '-day-col-' + Ext.Date.format(evtsd[i].date, 'Ymd');
    ...
    getDayAt: function(x, y) {
        var xoffset = this.el.down('.ext-cal-day-times').getWidth(),
            ...
            mins = rowIndex * 30,
            dt = criterion.ux.calendar.util.Date.add(this.viewStart, {days: dayIndex, minutes: mins}),
            el = this.getDayEl(dt),
            timeX = x;
    ...
    onClick: function(e, t) {
        if (this.dragPending || criterion.ux.calendar.view.DayBody.superclass.onClick.apply(this, arguments)) {
