 ...
 */
Ext.define('criterion.ux.calendar.util.WeekEventRenderer', {

    requires: ['criterion.ux.calendar.util.Date'],
        ...
        render: function(o) {
            ...
            for (; w < weekCount; w++) {
                if (!grid[w] || grid[w].length == 0) {
                    ...
                    dt = criterion.ux.calendar.util.Date.add(dt, {days: 7});
                } else {
                    var row,
                        d = 0,
                        wk = grid[w],
                        startOfWeek = Ext.Date.clone(dt),
                        endOfWeek = criterion.ux.calendar.util.Date.add(startOfWeek, {days: dayCount, millis: -1});
                        ...
                        if (wk[d]) {
                            ...
                            for (; ev < ct; ev++) {
                                ...
                                if (!evt.isSpan || evt.isSpanStart) {
                                    ...
                                    item._weekIndex = w;
                                    item._renderAsAllDay = item[criterion.ux.calendar.data.EventMappings.IsAllDay.name] || evt.isSpanStart;
                                    item.spanLeft = item[criterion.ux.calendar.data.EventMappings.StartDate.name].getTime() < startOfWeek.getTime();
                                    item.spanRight = item[criterion.ux.calendar.data.EventMappings.EndDate.name].getTime() > endOfWeek.getTime();
                                    ...
                                    var diff = criterion.ux.calendar.util.Date.diffDays(dt, item[criterion.ux.calendar.data.EventMappings.EndDate.name]) + 1,
                                        cspan = Math.min(diff, dayCount - d);

                                    if (cspan > 1) {
                                        cellCfg.colspan = cspan;
                                    }
                                    var t = Ext.core.DomHelper.append(row, cellCfg);

                                    if (evt.data['-isTimeoffObj']) {
                                        var isFullDay = evt.data['-isFullDay'];

                                        new Ext.tip.ToolTip({
                                            cls : 'ext-cal-day-tooltip',
                                            target : t,
                                            width : 200,
                                            html : Ext.util.Format.format(
                                                '<div class="tooltipBody">' +
                                                    '<div class="employeeName">{0}</div>' +
                                                    '<div class="timeoffType"><div class="timeoffTypeMark" style="{1}"></div>{2}</div>' +
                                                    '<div class="fullDay {3}">{4}</div>' +
                                                    '<div class="time {5}">{6}</div>' +
                                                    '<div class="date">{7}</div>' +
                                                    '<div class="noteTitle">{8}</div>' +
                                                    '<div class="note">{9}</div>' +
                                                '</div>',
                                                evt.data['-employeeName'],
                                                /^#/.test(evt.data['-color']) ? 'background-color: ' + evt.data['-color'] : '',
                                                evt.data['-timeoffDesc'],
                                                (isFullDay ? '' : 'x-hidden'),
                                                (isFullDay ? i18n.gettext('Full Day') : ''),
                                                (isFullDay ? 'x-hidden' : ''),
                                                Ext.Date.format(evt.data['-startTime'], criterion.consts.Api.SHOW_TIME_FORMAT) + ' -> ' + Ext.Date.format(evt.data['-endTime'], criterion.consts.Api.SHOW_TIME_FORMAT),
                                                Ext.Date.format(evt.data['-startTime'], 'D, j M'),
                                                i18n.gettext('Note'),
                                                evt.data['-timeoffNote']
                                            ),
                                            trackMouse : true
                                        });
                                    }

                                    Ext.isFunction(o.renderCallback) && o.renderCallback(item);
                                }
                            }
                            if (ev > max) {
