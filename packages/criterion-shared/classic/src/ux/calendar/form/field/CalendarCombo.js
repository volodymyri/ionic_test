/**
 * @class criterion.ux.calendar.form.field.CalendarCombo
 ...
 */
Ext.define('criterion.ux.calendar.form.field.CalendarCombo', {
    ...
    fieldLabel : i18n.gettext('Calendar'),
    triggerAction: 'all',
    queryMode: 'local',
    forceSelection: true,
    editable: true,
    ...
    listConfig : {
        getInnerTpl: function(displayField) {
            return '<div class="x-combo-list-item ext-color-{' + criterion.ux.calendar.data.CalendarMappings.CalendarId.name +
                '}"><div class="ext-cal-picker-icon">&#160;</div>{' + displayField + (this.pickerField.encodeHtml ? ':htmlEncode' : '') + '}</div>';
        }
    },

    // private
    initComponent: function(){
        this.valueField = criterion.ux.calendar.data.CalendarMappings.CalendarId.name;
        this.displayField = criterion.ux.calendar.data.CalendarMappings.Title.name;
    ...
    getStyleClass: function(value){
        ...
        if (!Ext.isEmpty(val)) {
            ...
            return 'ext-color-' + (val.data ? val.data[criterion.ux.calendar.data.CalendarMappings.CalendarId.name] : val);
        }
        return '';
    },
    ...
    setValue: function(value) {
        if (!value && this.store.getCount() > 0) {
            ...
            value = this.store.getAt(0).data[criterion.ux.calendar.data.CalendarMappings.CalendarId.name];
        }
