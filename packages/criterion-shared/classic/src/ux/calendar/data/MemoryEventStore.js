 ...
 */
Ext.define('criterion.ux.calendar.data.MemoryEventStore', {
    extend: 'Ext.data.Store',
    model: 'criterion.ux.calendar.data.EventModel',
    ...
    constructor: function(config){
        ...
        this.sorters = this.sorters || [{
            property: criterion.ux.calendar.data.EventMappings.StartDate.name,
            direction: 'ASC'
        }];
        
        this.idProperty = this.idProperty || criterion.ux.calendar.data.EventMappings.EventId.mapping || 'id';
        this.fields = criterion.ux.calendar.data.EventModel.getFields();
    ...
    interceptCreateRecords: function(records, operation, success) {
        if (success) {
            ...
            for (; i < len; i++) {
                records[i].data[criterion.ux.calendar.data.EventMappings.EventId.name] = records[i].id;
            }
        }
    },
